...
WARNING: The way you construct your service can have significant impacts. In this situation, we said *update*, but *replace* is a better description. For example, if the name was NOT provided, it would instead get nulled out.
...
[source,java,tabsize=2,indent=0,highlight=61-62]
----
...
there is a "self" link (kind of like a `this` statement in code) along with a link back to the *https://www.google.com/search?q=What+is+an+aggregate+root[aggregate root]*.
