...
Next, we need to declare the details of our REST resource for bookmarks. For this we can create new  `ResourceSupport` and add links to it.
...
Earlier in this tutorial, we saw this controller working with `Bookmark` objects. This version now wraps them inside the `ResourceSupport` and `Resource`, bringing along the extra goodness of HATEOAS.
...
The `Resource` type *wraps* a `Bookmark` and provides a nice, centralized place to keep link-building logic.
...
[source,xml,indent=0]
----
include::{book-root}/security/pom.xml[tag=security]
----
